...
`D:\dev\NTP-Groups\dist\edge\NTP-Groups-0.2.1-edge.zip`

The package must be byte-identical to the Chromium 0.2.1 package and contains no `update_url` field.

## Current Partner Center submission defaults

### Packages

Upload: `D:\dev\NTP-Groups\dist\edge\NTP-Groups-0.2.1-edge.zip`
...
Configuration can be exported/imported as a local JSON backup with one-step import undo. The optional Persistent Hub keeps one pinned NTP Groups tab per normal browser window; the toolbar action or assigned Return to Hub shortcut returns to that Hub without replacing the current site tab, and manual Hub placement inside the pinned strip is preserved. Search is handed to the browser's configured default search provider through the browser search API.
...
Edge-specific sideload acceptance for **NTP Groups 0.2.1 passed** on Microsoft Edge 151.0.4129.86 using an isolated temporary profile and the real **Load unpacked** flow.
...
- Persistent Hub pins the selected NTP, leaves launched sites/searches in separate tabs, and returns through the toolbar/background command path. The default shortcut is `Alt+H`, user remapping is respected, and returning to Hub does not force the Hub back to a fixed pinned position.
...
The Edge and Chromium 0.2.1 ZIPs must be rebuilt byte-identically and verified before publication.
